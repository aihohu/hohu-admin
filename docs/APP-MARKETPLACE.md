...
**菜单声明（决策 #80 #81）**：低代码应用在 manifest 顶层用 `menu`（单 dict，向后兼容，1 入口）或 `menus`（数组，N 入口）声明。两者都写时 `menus` 胜出。前端 `buildContributeMenus` 按 `app_slug` 归组：1 menu 的 app 保持顶级；≥2 menus 的 app 自动合成一个父组（label = `app_name`），子项为各 menu。父组不可导航，order 取所有子项 min。这样 CRM 类多入口应用在 sidebar 上读作"一个系统"，而非 N 个并排顶级。
...
- `x-ref` 中也可通过 `"x-ref-label": "name"` 声明；存在显式 `relations[].label_field` 时以后者为准

**`x-ref` 与 `relations` 的边界**（避免重复声明）：
- 简单单关联场景：只在 `data_schema` 字段上写 `"x-ref": "customer"` 即可，系统自动推导 belongs_to 关系，无需在 `relations` 重复声明
- 需要显式 `label_field` 等元数据时可写 `relations`；其 `foreign_key` 字段必须同时声明指向同一目标的 `x-ref`
- `relations.model` 与字段 `x-ref` 不一致时拒绝 manifest，避免运行时关系与物理 BIGINT 存储契约分叉
...
79. **belongs_to 关联总是 auto-join，label 字段名 `<fk>_label`** — `DataApiService.list()` 解析 manifest 中的 `belongs_to` 声明（`model.relations[]` 优先；缺省时 fallback 到字段级 `x-ref` + 可选 `x-ref-label`），单次 batch `SELECT id, label_field FROM target WHERE id IN (...)` 拉所有 label，写回 `record[<fk>_label]`。理由：spec §6.5 说"list endpoint auto-JOIN"，前端不用记 `?expand=` 参数；扩展字段是只读派生，不影响 WHERE/COUNT/分页；N+1 用 batch + Python set dedup 防御。**反例**：要求前端传 `?expand=customer` —— 复杂度跳一档，且 99% 列表场景都需要 label，强制参数只增加心智负担。**回归**：显式 relation 的 foreign key 必须带一致 `x-ref`；label_field 缺省 → target 第一个 string 列；显式 label 必须命中物理字符串列；FK 指向已删除父行 → 空字符串；前端 TablePage 关联列读 `<key>_label` 而非 `<key>`；关联列**支持排序**（见决策 #81，JOIN-based）；x-ref 字段由统一类型映射和 schema comparator 按 BIGINT 处理（与 target.id BIGSERIAL 匹配，防 Snowflake ID 截断及重装误判）。详见 §6.5。
80. **多菜单支持：`manifest.menus` 复数优先于单数 `menu`** — `contributes_service.aggregate_for_tenant` 同时接受 `manifest.menu`（单 dict，向后兼容）和 `manifest.menus`（数组，N 个菜单/app）。两者都声明时 **`menus` 数组胜出**（避免歧义，与 `relations[]` 优先于 `x-ref` 的约定一致）。理由：早期 spec 假设 1 app = 1 menu，但实际 CRM 类应用需要客户管理 + 订单管理两个入口；强制开发者写多份 manifest 拆 app 是反模式。**反例**：要求"多入口 = 多 app"——破坏 model 共享、permission 共享、版本协同；manifest 应该一份数据 + N 个 UI 入口。**回归**：现有 `menu`（单数）的 manifest 不需改动，行为不变；新 manifest 用 `menus: [...]` 即可多入口。前端 `buildContributeMenus` 按 `app_slug` 归组（见决策 #81）。
81. **多菜单 app 自动归到 `app_name` 父组下** — 前端 `buildContributeMenus` 按 `app_slug` 聚合 contributes menus：1 menu/app → 顶级菜单（不嵌套，避免多余点击深度）；≥2 menus/app → 合成一个父级组（label = `app_name`，icon = 子项第一个有声明的 icon，routePath = `app-group:<slug>` 非导航），所有 menus 作为子项。理由：用户心智模型是"一个系统 = 一个父级"（如 CRM 系统下挂客户 + 订单），而不是"N 个并排顶级菜单"造成 sidebar 拥挤。**反例**：N 个顶级菜单——5 个 multi-model app 各 3 个 menu = 15 个顶级菜单，sidebar 难以浏览。**回归**：父级组不可点击导航（点只展开/折叠）；order 取所有子项 min（保证整组在 sidebar 位置合理）；单菜单 app 不强制嵌套。
82. **关联列 `<fk>_label` 支持 JOIN-based 排序** — `order_by=customer_id_label` 时，`DataApiService.list()` 检测到 token 是 `<fk>_label` 形式且有对应 belongs_to 关系，构造 `LEFT JOIN target_table sort_<fk> ON main.<fk> = sort_<fk>.id`，`ORDER BY sort_<fk>.<label_field> <direction> NULLS LAST`。理由：用户看到客户名列，按名排序是直觉；之前决策 #79 草案标 "Phase 1 不支持"过保守，本决策补上。**反例**：让用户用 `order_by=customer_id` 排数字 ID——名字顺序与 ID 顺序无关，反直觉。**回归**：多列混合排序支持（如 `customer_id_label,amount`，JOIN + 真实列并存）；JOIN 存在时 SELECT 用 `<table_name>.*` + WHERE 限定 `tenant_id` 为 `<table_name>.tenant_id` 避免 ambiguous column 错误；target 表不存在或无字符串列时返回 400；label 是派生列，COUNT 不受 JOIN 影响（belongs_to 不会 fan-out）。
83. **数字字符串 ID 在 create/update 强转 int** — 前端为防 JS BigInt 精度丢失，把 Snowflake ID 序列化为 JSON 字符串传输（如 `"customer_id": "7477..."`）。但 asyncpg 拒绝 str→BIGINT 自动转换，会 500。`DataApiService.create()` 和 `update()` 在执行前 introspect 列类型，对 integer/bigint 列上的纯数字字符串自动 `int()`。理由：保持前后端 ID 序列化约定（字符串传输）的同时不破坏 DB 类型；其他驱动（psycopg）能自动转，但项目用 asyncpg，规则严。**反例**：要求前端转 Number —— Snowflake ID 超 `Number.MAX_SAFE_INTEGER`，精度丢失，存的 FK 找不到原行。**回归**：仅对 `_NUMERIC_TYPES` 列强转，text/jsonb/date 列不动；非纯数字字符串保持原样让 PG 报错（防误转 `"-abc"` 之类）。
84. **表单校验按 schema 类型声明 `type`** — NaiveUI `async-validator` 不声明 `type` 时默认 `'string'`，对数字字段值 `123` 会触发 required 判定不一致（部分版本把数字当 "非字符串" = 不通过）。FormPage 按 schema 字段类型显式声明：integer/number → `'number'`，boolean → `'boolean'`，array → `'array'`，object → `'object'`，其他 → `'string'`。理由：JSON Schema 已声明类型，校验器应一致；显式 type 还能多挡一道"数字字段传字符串"的脏数据。**反例**：留 async-validator 默认 string —— 用户输入 `0` 被认为"非字符串"通不过 required，体验崩。**回归**：FormPage 还在 mount 时把所有字段预初始化为 `null`（或 default），让 Vue reactivity 从一开始就追踪 key，避免动态加 key 绕过 NForm 的 change-trigger。
85. **Phase 1 SafeHttpClient 范围** — Phase 1 仅实现 `app/utils/safe_http.py::SafeHttpClient` 类（GET-only + 协议白名单 + URL pattern 白名单 + IP 黑名单含 `0.0.0.0/8` + IPv4-mapped IPv6 双栈校验 + `follow_redirects=False` + 1MB 响应上限 + connect 5s/read 10s 超时），不接入低代码 action 执行器。**理由**：前端动作按钮尚未实现，SafeHttpClient 是独立可测的安全模块；待 action / `x-external-ref` 落地时直接 `import` 调用即可。**反例**：把 SSRF 校验逻辑内联到 action executor 里，导致 action 重构时安全逻辑被牵连改动；或把 DNS 解析用同步 `socket.getaddrinfo`，阻塞 async 事件循环。**回归**：`test_safe_http.py::TestValidateIp` / `TestScheme` / `TestBlockedUrls` 覆盖 spec §SSRF Phase 1 全部 7 条要点；spec §6.5 line 968 文字描述里的 `0.0.0.0/8` 段必须进黑名单（伪代码遗漏，按文字描述实现）。
86. **Core MVP 整体延期 Marketplace/Lowcode** — 现有源码和测试作为后续迭代基础保留，但 Backend 不挂载 Marketplace、Contributes、app-data router，Web 在路由/菜单注册前统一过滤 Marketplace 与 `/app` Lowcode 路由。**反例**：只关闭后端，仍让 Web 展示可点击入口并产生一串 404。**回归**：所有 tenant mode 下前后端路由表均不暴露延期入口；恢复必须通过独立 spec、风险复核与发布画像审批。
