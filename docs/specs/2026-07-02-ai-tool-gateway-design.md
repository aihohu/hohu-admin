...
| chat 内直接上传文件 chip + 注入 file_id（SR-25） | （本 commit） | `chat-input.vue` 改 📎 按钮（accept 含 .csv/.xlsx/.xls）+ chip 预览；store `attachedFiles` + addFile/removeFile/clearFiles；sendMessage 拼 injectText 传 doStream；前端发 displayContent 字段，后端 chat.py 持久化优先用 display 版（LLM 仍看注入版）；Playwright 验证：用户 bubble 仅显示原始「解析这个文件」，LLM 自动调 file.parse 返回完整 markdown 表格 |
