...
| ✅ **跨会话 HITL 恢复 — 已完成 2026-07-18**（spec §14 / SR-14） | 刷新页面 / 关浏览器重开 / 跨设备 / 长时间挂页面都丢抽屉 | DB 主查 `ai_operation_log WHERE user_id+status=pending_confirmation` + Redis GET 校验每个还活着（防 DB 脏数据）；前端 onMounted 拉一次 + 30s 心跳；顶部 Banner 提示 + 点击 attemptResume 复用现有 SR-9~12 SSE 续传链路 |
...
#### SR-14. **跨会话 HITL 恢复用 DB 主查 + Redis GET 校验双源（不纯 Redis SCAN）**（2026-07-18 v1.5+ 落地，spec §14）— `GET /ai/pending-confirmations` 先 DB 拿 `ai_operation_log WHERE user_id=? AND status='pending_confirmation'`（按 queued_at 降序），再对每行 Redis GET pending payload 校验还活着（防 DB 脏数据——Redis 已 expire 但 DB 状态未迁移时跳过）。前端 onMounted 拉一次 + 30s 心跳（跨设备感知）；顶部 Banner 提示 + 点击 attemptResume 第一个复用现有 SR-9~12 SSE 续传链路；attemptResume/approve/reject 时立即从列表移除（避免心跳再次提示）。
**反例**: (1) 纯 Redis SCAN（`scan_iter(match="ai:confirm:*")` + JSON parse + user_id 过滤）→ O(N) 扫全 key，客户量大时 Redis 阻塞。(2) 纯 DB 查 → Redis 已 expire 但 DB 状态还是 pending_confirmation 时（服务重启清扫 Redis 但 mark_expired_if_pending 还没跑），返回的 confirmation_id 调 resume 会 404，UX 差。(3) Banner 显示 args 详情 → 多余（用户要先点开看）+ 潜在敏感字段泄漏（args 含 user_names / phones）。
**回归**: DB 主查有索引（user_id+status）；Redis GET 每confirmation 1 次（N 通常 ≤ 几个）；列表仅返 `args_summary` 元信息（不含 args 原值），args 详情走 attemptResume → SSE confirmation_required 流获取；前端心跳跳过条件 `pendingConfirmation != null`（避免抢焦点）；attemptResume 失败静默降级（D6）不阻塞用户。
