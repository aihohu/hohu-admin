...
60. **Task 14 Role-Agent 页面写入使用完整 Role Delegation Policy** — `PUT /ai/role-agent/{role_id}` 把输入视为严格、去重的 canonical Snowflake `string[]` 完整集合；普通管理员必须持有 `system:role:ai-agent-auth`，目标 Role 的 permission/menu/data scope、自定义部门和旧/新 Agent 并集均受 `GrantAuthority` 支配，全部成员必须处于 user scope，且每个成员变更前后的完整角色组合、有效 permission/menu/Agent 与物化 user/dept scope 都受支配。`R_SUPER`、admin/R_SUPER 成员、自改和任何全局影响越界均 fail closed；写入按 role → dept → user 锁定后重建同一快照，漂移统一 `AUTHORIZATION_SNAPSHOT_STALE`。超级管理员保留显式旁路，但只锁操作者授权依赖和目标 Role，不扫描目标 Role 全体成员。Web 对 shared Agent 采用显式可勾选绑定，不再恢复历史隐式直通。**反例**: 只校验新 Agent 会允许普通管理员借完整替换删除自己无权委派的旧绑定；只校验目标 Role 不校验成员最终权限会把越权授权扩散给混合角色成员；超级管理员扫描 `R_USER` 全体成员会把正常恢复操作放大为长事务。**回归**: `tests/modules/ai/test_role_agent_delegation.py`、`tests/modules/ai/test_role_agent.py`、`src/views/system/role/modules/__tests__/ai-agent-auth-modal.spec.ts`、`tests/e2e/phase2-role-agent-delegation.spec.ts`。
...
- [x] ✅ Task 14 已完成（2026-08-18）：`/ai/role-agent` 普通管理员写入口接入完整 Role Delegation Policy，覆盖严格完整集合、旧/新 Agent 委派上界、目标 Role 定义、全体成员 user scope、保护身份、自改、成员变更前后完整有效授权、稳定锁序和锁后快照复验；超级管理员保留轻量显式旁路。Web shared Agent 改为显式绑定，并补齐稳定错误码映射。真实 Chrome 多角色 E2E 创建“入口角色 + Agent 上界角色 + 目标角色”的普通管理员，验证合法 `user_mgmt` 绑定成功、越权 `dept_mgmt` 原子拒绝以及重开页面后状态不漂移。Phase 1 + Phase 2 安全集成门禁已通过，但仍只允许进入集成分支，不允许生产部署。
- [x] Task 14 / Phase 1 + 2 集成验证证据：后端 Role-Agent 定向 27 项及全量 2139 项测试通过，总覆盖率 74.89%；`ruff check .`、`ruff format --check .`、22 tools / 12 static checks 通过，仅保留 32 条既有 warning。Web `pnpm fmt`、lint（0 error / 30 条既有 warning）、typecheck、31 files / 121 tests、production build 与完整 `pnpm e2e`（Chrome 14/14）通过；既有聊天 E2E 已按 fail-closed 可用性门禁提供显式 Agent/安全模型，管理员 Role-Agent 用例不再依赖 `R_USER` 位于第一页。本工作包无数据库 migration、权限/menu seed 或新权限码；Phase 3 writer 收口、Phase 4 Trace/全局前端覆盖率/真实 provider release smoke 仍未完成。
- [x] ✅ Phase 2 全面审查修复（2026-08-20）：用户精确查找多命中 fail closed、部门精确计数冻结全部授权贡献对象、Role-Agent Service 授权先于资源存在性查询；Web 同步 `AI_LOOKUP_AMBIGUOUS` 中英文错误映射。无数据库 migration、权限/menu seed 或新权限码。
- [x] Phase 2 全面审查验证证据：新增 4 项后端失败回归转绿，Phase 2 授权/Tool/投影定向回归 162 项和后端全量 2143 项通过，总覆盖率 74.91%；`ruff check .`、`ruff format --check .`、22 tools / 12 static checks 通过，仅保留 32 条既有 warning。Web format、lint（0 error / 30 条既有 warning）、typecheck、31 files / 121 tests、production build 与完整 Chrome E2E 14/14 通过。
