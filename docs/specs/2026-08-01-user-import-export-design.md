...
- [ ] Task 22b **[v2.2 P0/P1/P2 验收补丁]**: 补 Task 22a 审计发现的 7+1 测试缺口（按风险排序）：
  - **P0 高风险（必补）**: `test_concurrent_execute_same_batch`（asyncio.gather 10 并发 → CAS 仅 1 成功）/ `test_preview_cache_missing_falls_back_to_db`（Redis 全丢 → DB 反查 execute 仍成功）/ `test_preview_cache_corrupted_falls_back_to_db`（Redis value 篡改 → DB 反查 execute 仍成功）
  - **P1 中风险（建议补）**: `test_execute_same_token_twice_running_concurrent`（concurrent RUNNING 重放抛 `AI_IMPORT_BATCH_RUNNING`）/ `test_execute_expired_batch_rejected`（EXPIRED 重放抛 `AI_IMPORT_ALREADY_EXECUTED`）/ `test_log_records_fatal_error`（模拟 OperationalError → EXECUTE_FAILED event）/ `test_state_created_to_failed_on_parse_error`（dry_run parse error → CREATED 直接 FAILED）
  - **P1 低风险（可选补）**: `test_log_records_chunk_progress`（2000 行 → 20 条 CHUNK_PROGRESS 行计数）/ `test_export_audit_chain_joinable_with_operation_log`（schema JOIN 单测）
  - **静态防回归（建议加 pre-commit）**: `tests/test_no_import_preview_session.py`（grep codebase 无 `class ImportPreviewSession` 残留）
  - **不在 Task 22b 范围**: `test_ai_import_preview_*` 4 测试等 Task 26/26a 实现 + `test_ai_export_always_creates_task` 等 Task 27 实现 + `test_business_layer_independent_of_storage_impl`（ruff TID251 lint 已等价强制）+ `test_state_db_check_rejects_invalid_value`（Postgres ENUM schema constraint 等价）+ `test_import_accepts_exactly_2000_rows`（`> 2000` 拒绝的自然推论）
