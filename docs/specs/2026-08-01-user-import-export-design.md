...
- [ ] Task 0a **[v2.2 P1 #2.2]**: 建 `app/modules/system/user/` 子包骨架（service.py / constants.py / schemas.py / models.py / import_service.py / import_parser.py / import_validator.py / import_state.py / export_service.py 9 个空文件 + `__init__.py` 重导出 facade）+ 旧 `service/user_service.py` 暂保留
- [ ] Task 0b **[v2.2 P0 #2.26 + P1-2]**: `ImportBatchStatus(Enum)`（含 `PREVIEW_DONE` v2.2 P1-2）+ `EmployeeNoSyncMode(Enum)` + `ExportTaskStatus(Enum)` + `LEGAL_TRANSITIONS` 映射（含 CREATED → PREVIEW_DONE / FAILED 分支）+ `_transition_batch_status` CAS helper（在 `import_state.py`）+ 单测（含 `test_state_illegal_transition_rejected` / `test_state_cas_prevents_race` / `test_state_created_to_preview_done` / `test_state_preview_done_to_running`）
- [ ] Task 0c **[v2.2 P0 #2.10]**: `USER_IMPORT_MAX_ROWS = 2000` / `USER_EXPORT_ASYNC_THRESHOLD = 5000` / `MAX_PREVIEW_RECORDS = 2000` 常量（在 `constants.py`，**不进 settings 防误改**）
- [ ] Task 0d **[v2.2 P1-4 #3.9]**: `app/core/file_storage.py` 模块：`FileStorage` Protocol + `LocalFileStorage` 默认实现 + `MockFileStorage`（测试用）+ `get_file_storage()` DI 工厂 + `settings.FILE_STORAGE_BACKEND` 配置项 + lint 规则禁止业务层 import 具体类 + 单测（5 用例）
- [ ] Task 0e **[v2.2 P1-3 #2.30]**: 定义 `ReasonSchema` Pydantic mixin（`reason: str = Field(min_length=1, max_length=256)`）+ `AI_IMPORT_REASON_REQUIRED` / `AI_EXPORT_REASON_REQUIRED` 错误码 + reason 一致性校验 helper（preview vs execute 比对）
