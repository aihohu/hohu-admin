...
> 状态：✅ Phase 1 + Phase 2 已完成（2026-08-05，Task 0a-34 全部 ship，覆盖率 70.48%）/ ✅ v2.4 Task 35 Safety Gate 已完成（2026-08-07）/ ✅ v2.5 Task 35a 已完成（2026-08-11，真实 Chrome Playwright 4/4）/ ⚠️ Task 36 容量证据待实施 / ⚠️ 完整 Phase 3 明确推迟到 `ai-tool-gateway v1.5+` 重新评估，不建设异步任务基础设施 | 创建日期：2026-08-01 | v2 修订：2026-08-01 | v2.1 修订：2026-08-01 | v2.2 修订：2026-08-03（含 P1 二次细化） | v2.4 边界修订：2026-08-06 | v2.4 Safety Gate 实施：2026-08-07 | v2.5 确认编排修订：2026-08-07 | v2.5 主实现：2026-08-08 | v2.5 验收完成：2026-08-11
> 作者：Jack
...
AI 确认抽屉的国际化由 Web 按 `tool + semantic label/value` 映射完成：`user.import_execute` 的稳定字段标识 `total/new/exists/conflict/outOfScope/onConflict/syncMode` 和策略枚举复用 `common.importModal.*` 文案，工具名与摘要也使用同一命名空间；后端 action 继续持久化安全 canonical presentation 作为审计/兼容回退，不把 locale 加入授权事实。未知工具、字段或枚举必须原样回退。**反例**: 前端直接把后端冻结的中文 title 当唯一显示源 → 用户切换语言或 reload pending 后仍显示旧语言；前端把所有未知 label 都当 i18n key → 新工具上线时显示空白。**回归**: 中英文 callback 对已知字段/枚举得到本地化文本，数值及未知字段/值保持原样，切换 locale 后 computed presentation 即时重算。

`user.export` 使用相同规则：工具名复用 `common.exportModal.title`，摘要由 `affectedCount` 注入 `common.exportModal.aiConfirmSummary`，`reason/affectedCount` 字段分别显示为业务理由/预计导出行数；reason 的值是用户输入，不翻译，行数保持数字。**反例**: 用正则翻译整段后端中文 summary → 数量、保留期或文案稍变即失效；翻译 reason 值 → 改写用户实际批准的业务理由。**回归**: 中英文动态摘要保留 count，reason 原文不变，missing count 时仍有可读回退。
...
| `test_export_audit_chain_joinable_with_operation_log` | sys_operation_log（path + request_params.reason + user_id）↔ sys_user_export_task（reason + operator_id + created_at）可对照反查（v2.3 修订：sys_operation_log 无 export_id 字段，实际 JOIN 走 reason + operator_id + 时间窗） |
...
- [x] Task 1 ✅ 已完成（2026-08-03）：14 个 Pydantic schema 落 `app/modules/system/user/schemas.py`（行号映射：`_CamelBase` 27 / `ReasonSchema` 33 / `UserImportRecord` 62 / `FailedRow` 87 / `ImportDryRunResult` 100 / `ImportResult` 140 / `UserExportFilter` 168 / `UserExportRequest` 179 / `UserExportTaskQuery` 203 / `UserImportBatchQuery` 220 / `UserImportBatchResponse` 247 / `UserImportBatchLogItem` 309 / `UserImportBatchCancelResponse` 336 / `UserExportTaskResponse` 357）。决策 1.1-1.6：
  ...
  - 1.6 **`FailedRow` 含 `row_index: int` + `error_code: str` + `error_message: str` + `raw: dict`** — `row_index` 让前端表格显示「第 N 行」+ 排序；`error_code` 是 i18n key 让前端 `errorCode.AI_IMPORT_*` 映射；`raw` 是原始行数据方便用户对照 Excel 找错。**反例**: 只返 `error_message: str` → 前端无法 i18n + 无法排序 + 无法对照原始 Excel。**回归**: `schemas.py:87` `FailedRow`；Task 10 service 层 IntegrityError → FailedRow(error_code='AI_IMPORT_DUPLICATE_EMPLOYEE_NO')；Task 17 前端表格列 row_index + i18n message。
- [x] Task 2 ✅ 已完成（2026-08-03，migration `0b2165376771`）：单个 alembic migration 创建 3 张新表 + 1 个字段 + 1 个 UNIQUE 索引：`sys_user_import_batch`（PostgreSQL ENUM status 含 PREVIEW_DONE + CHECK 约束 + reason 字段，spec §2.26 / v2.2 P1-3 #2.30）+ `sys_user_import_batch_log`（FK `batch_id` REFERENCES `sys_user_import_batch(id) ON DELETE CASCADE`，spec §2.28）+ `sys_user_export_task`（spec §2.31）+ `sys_user.employee_no VARCHAR(32)` + `UNIQUE INDEX ix_sys_user_employee_no`（spec §2.24）。决策 2.1-2.6：
  - 2.1 **单个 migration 而非 4 个** — Task 2 spec 描述了 3 张表 + 1 字段，本来可以拆 4 个 migration（每张表一个）。实际合成 1 个 `0b2165376771_add_user_import_batch_batch_log_export_` 因为：(a) 同一 PR 落地，无独立部署窗口；(b) 4 张表有 FK 依赖（batch_log → batch），分 migration 要拆 down_revision 链；(c) Task 0b 状态机 ENUM 类型 `importbatchstatus` 必须与 `sys_user_import_batch.status` 列同期创建，否则 status CHECK 约束引用不存在的类型。**反例**: 4 个 migration → 部分回滚时 batch 表存在但 batch_log 不存在 → FK 引用悬空。**回归**: `alembic/versions/0b2165376771_*.py` 单文件 4 个 op.create_table + 1 个 op.create_index + 1 个 ENUM type。
  - 2.2 **PostgreSQL ENUM 类型 + CHECK 约束** — `status` 列用 `postgresql.ENUM(name='importbatchstatus')` 而非 `VARCHAR + CHECK`。理由：(a) PostgreSQL ENUM 在 DB 层强类型校验，写入非法 status 直接报错；(b) 比 CHECK 约束快（查询规划器知 enum 范围）；(c) SQLAlchemy 反射时自动映射 Python Enum。迁移含 `op.create_type` + `op.drop_type`（drop 时反向）。**反例**: VARCHAR(20) + CHECK → 任何字符串都能 INSERT，CHECK 是事后校验，性能略差。**回归**: migration 用 `sa.enum(..., name='importbatchstatus', create_type=True)`；Task 0b Python Enum 值与 DB enum 值一一对应；`LEGAL_TRANSITIONS` 在 Python 层额外约束（双保险）。
  - 2.3 **`sys_user_import_batch_log.batch_id` ON DELETE CASCADE** — 删除 batch 时关联日志自动级联删除（GDPR 清理 / 测试清理场景）。**反例**: ON DELETE RESTRICT → 测试无法直接 DELETE batch，必须先删 log → 测试 fixture 复杂；GDPR 用户数据清理脚本要写两步。**回归**: migration `ForeignKeyConstraint(['batch_id'], ['sys_user_import_batch.id'], ondelete='CASCADE')`；Task 22 `cleanup_expired_batches` cron 直接 DELETE batch，log 自动级联；测试 fixture 用 `_transition_batch_status` raw UPDATE 不依赖 ORM cascade。
  - 2.4 **`sys_user.employee_no` UNIQUE 索引但允许 NULL** — PostgreSQL 默认 `UNIQUE` 允许多个 NULL（spec 兼容旧用户 employee_no 为空），但非空值必须唯一。**反例**: NOT NULL 约束 → 旧用户全部需要回填 employee_no 才能 migrate，迁移阻塞。**回归**: migration `sa.Column('employee_no', sa.String(32), nullable=True)` + `op.create_index('ix_sys_user_employee_no', 'sys_user', ['employee_no'], unique=True)`；Task 7a `resolve_existing_user` 用 `employee_no IS NULL` 判断走 username 分支；测试 fixture 显式 SET NULL 避免触发 UNIQUE。
  - 2.5 **alembic autogenerate 漂移修剪** — `alembic revision --autogenerate` 检测到 `ai_message` / `ai_model` / `ai_operation_log` / `role_ai_agent` 等表的 comment / index 漂移（ORM ↔ DB schema 不同步，历史遗留）。这些漂移与 Task 2 无关，已手动从 migration 删除（migration 头部注释明确记录）。**反例**: 不修剪 → migration 顺手把 AI 模块表结构也改了 → 与 Task 2 范围无关 + 影响 AI 模块测试 + 失去单一职责。**回归**: migration 文件头 docstring 第 14-17 行明确记录「已修剪」+ 列出漂移表清单；后续 AI 模块重构时单独 migration 处理。
  - 2.6 **`down_revision = 'fba0cf4a5e82'`（ON DELETE 策略 + tenant）** — 选 `fba0cf4a5e82_add_ondelete_strategies_and_tenant_` 作为父 migration，因为 Task 2 的 FK CASCADE 依赖父 migration 的 `ON DELETE` 策略基础设施（PostgreSQL 14+ 才完整支持 CASCADE on FK）。**反例**: 接 `init_database` → 跳过 ON DELETE 策略 → CASCADE 行为不一致（旧 PG 版本可能 IGNORE）。**回归**: migration `down_revision = "fba0cf4a5e82"`；Task 7 后续 `00558ec10892_add_records_hash_to_sys_user_import_` 接 `0b2165376771` 形成 linear chain。
...
- [x] Task 8 **✅ 已完成（2026-08-03）**：`import_parser.parse_import_excel`（模块级纯函数，零 DB 查询；service 层 thin wrapper 留 Task A）+ 24 用例（覆盖 spec line 2624-2630 全部 + v2.2 P0 `test_too_many_rows_raises` 拒绝 > 2000 行 + 边界：employee_no 空串 → None / gender status 默认值 / 多错误一次性收集）。
  ...
  - **决策 8.6**: value-only 导入允许忽略 openpyxl 不支持的 Excel Data Validation extension，并只在 `_parse_xlsx_rows` 的 `load_workbook` 边界精确抑制该条 `UserWarning`。**理由**: 导入只读取单元格值，既不消费验证规则也不保存 workbook；扩展从内存模型移除不会改变导入结果。**反例**: 全局忽略所有 openpyxl/UserWarning → 损坏文件或其他兼容性问题被静默；读取后另存用户文件 → 验证扩展真的会丢失，不允许复用此策略。**回归**: 含 `{CCE6A557-97BC-4B89-ADB6-D9C93CAAB3DF}` extension 的真实 OOXML 可解析且不产生该 warning；其他 warning 不被过滤。
...
- [x] Task 17 ✅ 已完成（2026-08-04，2026-08-06 修 NUpload max=1 bug）：前端 `user-import-modal.vue` 3 步弹窗（4 卡片统计 + 错误清单前 20 条 + sync_mode radio + cancel 按钮 + idempotentReplay toast）+ vitest。决策 17.1-17.7：
  ...
  - 17.7 **NUpload 去 `max=1`（2026-08-06 ship-time fix）** — Task 17 ship 时 NUpload 声明 `:max="1"` 想限制单文件，但实测发现「reason 为空 → 点 dragger → file chooser 选文件 → REASON_REQUIRED notifyError → 填 reason → 再点 dragger 不触发 file chooser」bug。根因：NUpload 内部 fileList 第一次选文件后 length 已 = 1，达到 max 上限后 input.click() 不再触发，用户卡死无法重选。修复：删除 `:max="1"`（`user-import-modal.vue:148`）。**理由**: `:show-file-list="false"` + step v-if 切换（step=2 卸载整个 NUpload）已保证 UX 层单文件语义；`:max="1"` 是冗余约束反而误伤 UX。**反例**: (1) 保留 max=1 + 在 handleFileChange 校验失败时调 NUpload ref.clear() → 多 1 个 ref + watch + clear 调用，改动大；(2) 保留 max=1 + 用 reason 控制 NUpload disabled → 用户必须先填 reason 才能选文件，UX 限制更死。**回归**: 浏览器手动复测「reason 为空 → 提示 → 填 reason → 再点 dragger → file chooser 触发 → dry_run 成功」全过；use-import-flow.spec.ts 10 用例 0 regression；commit `8a772588`。
...
- [x] Task 21 ✅ 已完成（2026-08-04 ship，2026-08-06 dev stack E2E + 浏览器手动验证跑通）：前端 lint（0 errors，31 pre-existing warnings 无关本 PR）+ typecheck（vue-tsc --noEmit --skipLibCheck 0 errors）+ vitest（5 files / 28 tests passing）+ Playwright E2E spec（`tests/e2e/user-import-export.spec.ts` 3 smoke 用例：导入按钮 / 导出弹窗 reason 校验 / 历史抽屉；2026-08-06 dev stack 实测 3/3 passing 11.7s）+ 浏览器手动验证 4 场景（3 步导入流程 / 导出 Blob 下载 / 历史抽屉分页 / AI user.export chip 下载；2026-08-06 dev stack 实测 4/4 passing）。决策 21.1-21.4：
  ...
  - 21.3 **lint warnings 全部 pre-existing，与 user-import-export 无关** — 31 warnings 来自 `table-header-operation.vue` / `ai/agent/index.vue` / `lowcode/widgets/*` 等历史模块（`vue/require-default-prop` + `vue/no-unused-properties` + `vue/no-undef-properties`）；本 PR 0 warnings 0 errors。**反例**: 在本 PR 顺手修历史 warnings → 范围扩张 + 模块 owner 不易 review；历史债务由对应模块 owner 在自己 PR 里清理。**回归**: `pnpm lint` 输出 31 warnings 全部位于非 user-import-export 文件；Task 21 不补历史模块的 prop default。
  - 21.4 **dev stack 实测 E2E + 浏览器手动验证（2026-08-06 ship-time 补跑）** — Task 21 ship 时只写了 E2E spec 文件未实跑（前置依赖 dev stack 起来）；2026-08-06 后端 `feature/user-import-export` + 前端 `feature/user-import-export-web` 分支合并前夕，本地起 dev stack（后端 :8000 + 前端 :9527）补跑两层验证：(a) `pnpm exec playwright test tests/e2e/user-import-export.spec.ts` 3/3 passing 11.7s（导入按钮 / 导出弹窗 reason 校验 / 历史抽屉）；(b) Playwright MCP 工具走 4 个浏览器场景：3 步导入流程（上传 import_template.xlsx → 预检显示 2 行 conflict + 4 卡片统计 + radio 选项 + 预检令牌 → 确认 → step 3 结果 + 默认密码提示 + 下载错误清单链接）/ 导出 Blob 下载（URL.createObjectURL hook 捕获 5405 bytes xlsx MIME + Playwright 保存 hohu-users-20260806-133649.xlsx）/ 历史抽屉分页（10 行批次列表 FAILED/PREVIEW_DONE/SUCCESS 状态色映射 + pagination 渲染 prev/next disabled）/ AI user.export chip 下载（HITL 抽屉 6 行倒计时 04:47 → 确认 → chat-tool-call 卡片渲染 ✓ user.export / high / 60ms / 6 行 + chip「⬇ 下载文件」→ Blob 5405 bytes + 保存 hohu-users-20260806-053858.xlsx）。**反例**: (1) 信 vitest 28 用例 + 静态 typecheck → 漏掉 NUpload max=1 UX bug（Task 17.7 ship-time 才发现）；(2) 信 E2E 3 smoke → 漏掉真实 AI tool 调用闭环（dry_run → HITL → execute → 下载），smoke 只覆盖按钮点击。(3) 不补 dev stack 跑 → PR CI 跑通不代表 dev 体验通，merge 后用户报 bug 才返工。**回归**: Playwright run output 3 passed 11.7s；4 个浏览器场景全部成功 + 下载文件落盘（5405 bytes xlsx）；ship-time 决策 17.7（NUpload bug）和 21.4 双向闭环。
...
- [x] Task 26a ✅ 已完成（2026-08-04）**[v2.2 P0 #2.14 拆分]**: AI tool `user.import_execute`（risk=high / hitl_always / dry_run_supported / rows_affected），强制走 HITL 防止 LLM 跳过 preview。决策 26a.1-26a.3：
  ...
  - **v2.5 纠偏说明**：26a.2/26a.3 是 2026-08-04 的历史实现事实，不再是目标编排；Task 35a 将其替换为 Gateway PreparedAction、Gateway-only execute 和已绑定 preview presentation，并在 35a.3 删除 `_dry_run_user_import_execute`，但保留 ImportBatch/hash/CAS 业务基础。
...
- [x] Task 34 ✅ 已完成（2026-08-05）：3 个 spec 要求但长期缺失的测试补齐（Task 22c+ 遗留）。决策 34.1-34.3：
  - 34.1 **test_ai_export_always_creates_task 显式断言 DB 行** — Task 27 决策 27.1 假设「service 层强制建 task → AI tool 复用就自动有了」，但 service 层测试覆盖的是 `export_users_to_excel` 直接调用入口，没有覆盖 AI tool 入口。本测试从 AI tool `user_export()` 入口触发，显式断言 DB 里 UserExportTask 行存在 + status=SUCCESS + row_count / reason / file_storage_key / file_size_bytes / filter_snapshot / 时间戳链路齐全。**反例**: 只在 service 层测 → AI tool 入口如果未来重构绕过 service（如直接调 `_build_excel`），审计链断；AI tool 入口是 LLM 可达面，必须显式验证。**回归**: `test_ai_tools_user_export.py::TestAlwaysCreatesTask::test_ai_export_always_creates_task`。
...
- [x] Task 35a **✅ 已完成（2026-08-11）**：按 ADR-0002 和 Gateway spec §4.7/§5.6/§8.8，用户导入已具备首个 `prepared + hitl + inline` 主链；确认 UI 由 Gateway 持久 action 驱动，不再依赖 Markdown 或 Prompt 触发。真实 Chrome 确定性 Playwright 4/4 已验证执行确认、刷新恢复、preview-only 和用户导出国际化边界；真实模型/provider 纵向 smoke 仍是独立发布验收，不由 route fixture E2E 代替。
  - [x] 35a.0 **✅ 已完成（2026-08-07）：先落共享因果与收口基础** — 已完成消息编辑 spec Task 1/2 及 Task 2b 的 send/direct-HITL 子集、工具卡 spec Task 0 和 Task 1/2 的基础子集：Web 在请求前生成稳定 traceId，后端验证并持久化原 trace/source/parent；conversation owner lease 使用 NX、heartbeat、pending TTL+grace 和 compare-owner release；正常 chat、confirm stream-gone、resume、timeout、startup cleanup 共用幂等 assistant finalizer，tool-only 也落库，`done(persistence=committed)` 只在 commit 后发送。edit/regenerate 与 PreparedAction 仍关闭，35a.1-35a.5 状态不变。**反例**: 直接在现有临时 SSE/PendingPayload 上叠 PreparedAction → action terminal、assistant card 和 guard 仍可能三方分叉；重复 confirm 把已经 running 的 turn 标失败并提前释放 guard。**回归**: migration 覆盖 active/revision/operation causality 与 assistant-run partial unique index；pytest 覆盖 stable trace/source、started 顺序、tool-only/idempotent finalizer、commit-before-done、offline/resume/startup cleanup、owned guard、pending heartbeat 不缩短 lease 和重复 confirm 不误收口；Vitest 覆盖客户端 trace 格式与唯一性。
    - 35a.0.1 **run 因果键由请求前 trace 与已 flush source message 共同定义** — `trace_id` 负责 retry/reconcile，`source_user_message_id` 负责 parent 与 operation attribution；二者不能由模型生成。**反例**: 流结束才生成 trace 或只存 conversation_id → ack 丢失后无法确认哪一轮已提交，多轮工具操作也无法绑定原用户消息。**回归**: `MessageOut.traceId`、operation source/readonly snapshot 与同 conversation trace reuse rejection 测试。
    - 35a.0.2 **terminal commit 是释放 owner lease 和发送 durable done 的前置条件** — Redis 只做互斥与 waiter，DB message/operation 才是恢复事实；HITL handoff heartbeat 必须保持 confirmation TTL+grace，不能缩回普通 60 秒 lease。**反例**: 先 done/释放再 commit → reload 读不到卡片且下一 command 与旧 run 重叠。**回归**: chat/resume/startup 顺序测试固定 `finalizer → commit → release → pending delete/done`；cleanup 失败只依赖 TTL 兜底，不伪装持久化失败。
  - [x] 35a.1 **✅ Gateway 自动接管 preview → confirmation 已完成（2026-08-07）** — 模型 schema 只暴露 `user.import_preview` 并强制选择 `requested_outcome`；Gateway 在调用业务 preview 前剥离保留字段，消费仅服务端可见的 `PreparedActionProposal`，对 `execute_if_approved` 自动进入绑定 `user.import_execute` 的结构化 HITL，不需要第二次 model tool call。`user.import_execute` 已从模型可见集合移除，tool/result/event 均不暴露 `preview_token`，确认事件只展示安全 presentation。35a.1 交付时仍复用现有 Redis PendingPayload/HITL waiter；PostgreSQL `PreparedAction` 事实源与 frozen hash 校验已在 35a.2 完成，CAS 与 reload 恢复留在 35a.5，因此 Task 35a 整体仍是 release blocker。**反例**: 强化 system prompt 要求“务必调用 execute” → 换模型/语言/上下文后仍可只输出文本；把 frozen args 塞回事件临时过渡 → token 泄漏到浏览器。**回归**: mock 模型只调用一次 preview，仍收到 `confirmation_required`；生产模型 schema 不含 execute；浏览器 E2E 显示 preview 成功后自动出现 pending 卡片和确认抽屉，拒绝后未创建用户。
  - [x] 35a.2 **✅ 导入策略在 preview 时一次冻结（2026-08-07）** — 新增 PostgreSQL `ai_prepared_action` 授权事实；Gateway 在 prepared preview 成功后持久化 `reason/on_conflict/sync_mode/preview_token`、args/snapshot canonical hash、batch/file/records hash、operator、可信 tenant、conversation/source message、prepare/execute call 和安全 presentation。`/ai/confirm` 请求只接受 `confirmationId + approve|reject` 且 `extra=forbid`；prepared flow 在 wake 前交叉校验 action 与 Redis payload，并对用户导入批次状态、token、reason、冲突策略和 preview snapshot 做最小业务复验。当前在线执行仍由原 Redis waiter 承接；action CAS 终态、权限/Data Scope/source active 全量复验、Redis 丢失恢复和 detail pending projection 仍由 35a.5 完成。**反例**: confirmation 请求再传 overwrite → 用户批准的 summary 与实际写入策略不同；只比较 Redis args 不比较持久 action → 内部 payload 被改写后仍可能执行。**回归**: approve body 只有 confirmationId/action；额外字段 schema 拒绝；frozen args/hash 与可信身份持久化；Redis args 换参拒绝；snapshot stale 在 wake 前拒绝且不执行。
  - [x] 35a.3 **✅ execute 已收口为 Gateway-only capability（2026-08-08）** — `user.import_execute.llm_visible=False`，模型 schema/available tools 中不存在；公共 `execute_tool` 不再暴露 prepared context 参数，绑定 execute 的普通直调返回 `AI_PREPARED_ACTION_REQUIRED`，只有 Gateway 私有路径能携带与 prepare source 匹配的 context。Registry 强制 target 为同域/shared direct tool、隐藏且 `hitl_always=True`；static gate 新增 prepared binding/visibility 两项检查并从 10 项增至 12 项。旧 `_dry_run_user_import_execute` 已删除，抽屉只使用冻结 preview presentation；preview token 只在服务端 frozen args/ImportBatch 中存在，不进 LLM、event、drawer 或 message JSON。**反例**: 只靠工具描述说“不要直接调用” → capability 仍暴露；公共 executor 留一个下划线 context 参数 → 任意内部调用可伪装 Gateway；保留二次 dry-run → 确认摘要双源漂移。**回归**: registry/static/runtime 三层不可达测试 + PydanticAI schema 过滤 + public signature/专用错误码测试；action CAS 与完整 approved context 仍由 35a.5 完成。
  - [x] 35a.4 **✅ 纯预览不弹窗且不可原地升级（2026-08-08）** — Gateway 把 `preview_only` 作为终止性公开投影：业务 preview 仍生成内部 `PreparedActionProposal`，但 Gateway 在返回前消费并丢弃 proposal，只保留结构化 `data/ui`；不创建 Redis pending、PostgreSQL `ai_prepared_action` 或 `confirmation_required`。用户之后改为执行时，只能以 `execute_if_approved` 重新调用 prepared preview，生成新的 tool call、业务快照和 action；旧 preview 结果没有可复用 capability，直调绑定 execute 继续返回 `AI_PREPARED_ACTION_REQUIRED`。**反例**: 所有 preview 都弹确认 → 用户明确只查看仍被打断；把 proposal/token 留在纯预览返回对象中供内部调用者复用 → 权限、策略、source 与业务快照可能已变化。**回归**: 集成测试同时断言 preview-only 只有 started/result 事件、结构化 summary、无 proposal/Redis pending/DB action；二次执行意图产生新的 preview tool call 并绑定新 action，旧结果直调 execute 被拒绝。
  - [x] 35a.5 **✅ 持久 pending 和一次性执行主实现完成（2026-08-08）** — PostgreSQL `ai_prepared_action` 是 prepared pending/授权/终态事实源；conversation detail 按 owner/tenant/active source 返回脱敏 `pendingActions`，Web 以 actionId 恢复确认卡。confirm 按 `conversation -> source message -> action` 加锁并以 row_version CAS 取得唯一执行权，批准时重新验证当前用户、tenant、agent、tool binding、权限、Data Scope、source、冻结 hash、ImportBatch snapshot 与私有 artifact；业务执行使用独立 session，Redis waiter 只接终态通知。reject/expiry/source stale/执行结果经共享 finalizer 在 DB commit 后释放 owner guard；启动时保留有效 pending、将残留 approved/running 收口为 interrupted，Redis flush 不能放行下一 ChatCommand。**反例**: waiter 被唤醒后再次执行业务 → 双击/多 worker 双写；终态只写 Redis 或先删 pending 再写 message → reload 丢卡；confirm body 重传参数或 presentation 展示 raw args → 批准对象漂移与 capability 泄漏。**回归**: pytest 覆盖 CAS、双确认只执行一次、无 Redis approve/reject、过期/source/tenant/owner、snapshot/artifact/current permission 复验和 startup recovery；Vitest 覆盖 detail 恢复、安全 DTO、失败保留 pending、running 轮询后的 durable handoff；真实浏览器验收结果记录于 Task 35a.6。
  - [x] 35a.6 **✅ 确定性真实浏览器验收已完成（2026-08-11）** — 在本地 dev stack 使用真实 Chrome 实跑 Playwright 4/4：执行意图自动弹窗并由 confirm API 完成执行、刷新后从 conversation detail 恢复同一 pending action、preview-only 只展示预览且不创建 action、用户导出确认抽屉显示国际化工具名/动态摘要/参数标签。测试使用真实登录、页面、store、drawer、detail reconcile 和 confirm request 边界；AI chat/detail/confirm 响应使用受控 route fixture，因此不声明真实模型/provider 端到端通过。**反例**: 把 fixture E2E 写成真实模型全链路通过 → 掩盖 provider、模型选 tool 和流式协议的独立风险。**回归**: `prepared-action-confirmation.spec.ts` 真实 Chrome 4/4，后续真实 provider smoke 单独记录结果。
  - [x] 35a.7 **✅ AI 用户导入确认展示国际化已完成（2026-08-11）** — Web 按 `user.import_execute + semantic label/value` 复用 `common.importModal` / `common.importHistoryDrawer` 中英文文案，工具名、摘要、统计字段、冲突策略与同步模式随当前 locale 即时渲染；服务端 action 仍保存安全 canonical presentation 供审计/兼容回退，未知工具/字段/枚举原样回退。**反例**: 持久化中文 title/label 作为唯一展示源 → 英文界面和切换语言后的 recovered pending 仍显示中文。**回归**: pure mapper Vitest 覆盖已知映射、数值保留和 unknown fallback；Vue typecheck 固定所有 i18n key 均存在。
  - [x] 35a.8 **✅ AI 用户导出确认展示国际化已完成（2026-08-11）** — `user.export` 工具名、带 `affectedCount` 的动态摘要、reason/预计行数字段复用 `common.exportModal.*` 中英文文案；用户填写的 reason 与数值不改写，原始 tool code 继续显示以便审计。**反例**: 正则替换后端中文 summary 或翻译 reason 值 → 文案变化即失配，且批准对象与用户原意不一致。**回归**: mapper Vitest 覆盖动态 count、本地化字段名、reason 原文和数值保留；missing count 使用本地化占位而不是抛错。
